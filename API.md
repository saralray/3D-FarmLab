...
{
  "version": "v1",
  "resources": ["printers", "queue", "analytics", "notifications", "slicer-keys", "audit-logs", "settings", "users", "admin-credential", "manager-requests", "maintenance", "filament-station", "status-light"]
}
```
...
Frontend mirror: `GET /api/status-light/provisioning` is **admin-only** (carries
the broker credential); `GET /api/status-light/devices` and
`GET /api/status-light/printers/:id` are **public reads** (no secrets — the
status string is not sensitive). The dashboard's printer-detail **Status Light
card** flashes and provisions a device in-browser over Web Serial.
...
web-container restart. It's still an estimate — excludes TLS/HTTP framing
overhead and any traffic nginx serves without reaching the app (e.g. the
Prometheus UI at `/prometheus`) — but both directions now count actual bytes:
outbound is Node counting response chunks, and inbound intercepts each
request's own `data` emissions (rather than adding a consuming listener, which
would switch the stream to flowing mode and risk dropping chunks before
downstream body parsing — busboy's file-upload stream above all — attaches),
so it's exact for whatever the route's handler actually reads.

| Method & path | Description |
|---------------|-------------|
| `GET /api/network-usage` | `{ today, monthToDate, daily[], byRoute[], poller[], processStartedAt }`. `today`/`monthToDate` are `{ bytesOut, bytesIn, requests }` — `bytesOut` is what the server sent (webcam snapshots, API responses, pages/assets), `bytesIn` is what it received (print-request/slicer uploads). `daily` is the last 30 calendar days, oldest first, zero-filled for days with no traffic, each point `{ date, bytesOut, bytesIn, requests }`. `byRoute` is the last-30-days total per route category (see `classifyRoute` in `server/metrics.js` for the vocabulary — `webcam`, `printer_proxy`, `api_v1`, `api_<resource>`, `static`, `app`, `healthz`, `readyz`, `metrics`), each `{ route, bytesOut, bytesIn, requests }`, sorted by `bytesOut` descending. `poller[]` is a **separate** traffic source — the poller's own last-cycle traffic to/from the printers themselves (HTTP/MQTT/FTP, not browser/client traffic), one entry per shard: `{ shard, shardCount, lastRunAt, cycleDurationMs, printersPolled, rowsWritten, refreshFailures, bytesOut, bytesIn }`; no history, since `poller_health` only keeps the latest cycle per shard. |
| `GET /api/network-usage/live` | `{ bytesOut, bytesIn, timestamp }` — cumulative-since-process-start totals (sum across all routes) plus the server's current epoch-ms time. Cheap, in-memory only (no DB query), meant to be polled every couple of seconds; the caller diffs two samples over the elapsed wall-clock time to derive a live bytes/sec rate, backing the Network Usage page's "Live" throughput indicator. Same admin gate as `GET /api/network-usage`. |
