...
// Slicer names Moonraker recognizes in a G-code "; generated by <name>" header.
// Moonraker only extracts a file's embedded thumbnails (which Klipper printers
// like the Snapmaker U1, and Fluidd/Mainsail, display as the print preview) when
// it can identify the slicer from this header. A branded fork name such as
// "3D-FarmLab-Orca-Slicer" isn't recognized, so the (perfectly valid) thumbnails
// are never parsed and the preview is blank everywhere.
const MOONRAKER_KNOWN_SLICERS = /^(OrcaSlicer|BambuStudio|PrusaSlicer|SuperSlicer|Slic3r|Cura|ideaMaker|Simplify3D|KISSlicer)/i;

// Rewrite the slicer name in the G-code's "; generated by <name> <version> on …"
// header to the canonical "OrcaSlicer" when it isn't already a name Moonraker
// recognizes, so Moonraker extracts the embedded thumbnails. The name is always
// a single whitespace-free token (recognized slicers and this fork's hyphenated
// name alike), so only that token is replaced — version/date are preserved.
// Only the header region is inspected (the line sits at the top); returns the
// buffer unchanged when there's no header or it's already recognized.
export function normalizeMoonrakerSlicerHeader(gcode) {
  const headLen = Math.min(gcode.length, 4096);
  // latin1 keeps string offsets equal to byte offsets (the header is ASCII), so
  // the splice below stays byte-aligned even if later bytes are multi-byte.
  const head = gcode.toString('latin1', 0, headLen);
  const match = /^; generated by (\S+)/m.exec(head);
  if (!match) return gcode;
  const name = match[1];
  if (MOONRAKER_KNOWN_SLICERS.test(name)) return gcode;
  const start = match.index + '; generated by '.length;
  return Buffer.concat([
    gcode.subarray(0, start),
    Buffer.from('OrcaSlicer', 'utf8'),
    gcode.subarray(start + Buffer.byteLength(name, 'latin1')),
  ]);
}
