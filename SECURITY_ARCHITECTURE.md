...
| S-2 | **Coarse authorization.** ~~Effective roles are `public / authed / operator / admin`~~ **(phase 1 landed.)** Replaced by an 8-role hierarchy (guest→super_admin) + a granular capability matrix + a single declarative route→capability registry (`server/rbac.js`), with the enforcement gate rewired to it and the scattered classifier removed. Tenant scoping helpers (`sameTenant`/`canAccessResource`) exist and are tested; the DB-level tenant columns + Postgres RLS are phase 2 (needs a DB to verify). | Now expresses per-role, per-capability control; tenant isolation model is in place awaiting the schema migration. |
...
- **Password hashing is server-side** (fixed). The SPA previously computed
  `sha256(password)` client-side and sent that — a replayable password-equivalent
  (pass-the-hash) with no benefit over TLS. The client now sends the plaintext
  over TLS and the server derives the hash (`submittedPasswordHash` → scrypt);
  the stored credential format is unchanged (so no lockout) and a legacy
  `passwordHash` body is still accepted for backward compatibility.
